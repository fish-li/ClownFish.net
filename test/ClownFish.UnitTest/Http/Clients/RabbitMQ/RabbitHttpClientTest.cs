...
public class RabbitHttpClientTest
{
    ...
    public async Task Test_SendMessage()
    {
        ...
        HttpClientMockResults.SetMockResult("Rabbit_QueueDeclare", ClownFish.Base.Void.Value);
        HttpClientMockResults.SetMockResult("Rabbit_QueueBind", ClownFish.Base.Void.Value);
        ...
        HttpClientMockResults.SetMockResult("Rabbit_GetMessage", message + "_这个API返回的是一个JSON__");
        ...
        HttpClientMockResults.SetMockResult("Rabbit_GetMessage", message2 + "_这个API返回的是一个JSON__");
        string result2 = await rabbitclient.GetMessageAsync(queue, false);
        Assert.IsTrue(result2.Contains(message2));


        HttpClientMockResults.SetMockResult("Rabbit_AckLast", ClownFish.Base.Void.Value);
        await rabbitclient.AckLast(queue);
        Thread.Sleep(1000);


        HttpClientMockResults.SetMockResult("Rabbit_MessageCount", "{\"messages\":0}");
