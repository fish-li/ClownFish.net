...
on a.object_id = b.object_id";
        ...
        string sql2 = @"
select top 1 * from sys.all_objects;
select top 1 * from sys.all_columns;
...
";
    ...
    public void Test_Parameter()
    {
        using( ConnectionScope scope = ConnectionScope.Create() ) {
            ...
            query.AddQueryParameter(new int[] { 2 });
            query.AddQueryParameter(new List<int> { 3 });

            query.AddQueryParameter(new long[] { 4 });
            query.AddQueryParameter(new List<long> { 5 });

            query.AddQueryParameter(new string[] { "aa" });  // @x1
            query.AddQueryParameter(new List<string> { "bb" });  // @x2

            Guid guid1 = Guid.NewGuid();
            Guid guid2 = Guid.NewGuid();

            query.AddQueryParameter(new Guid[] { guid1 });
            query.AddQueryParameter(new List<Guid> { guid2 });

            typeof(CPQuery).InvokeMember("SetParameter",
                BindingFlags.InvokeMethod | BindingFlags.Instance | BindingFlags.NonPublic,
                null, query, new object[] { "p1", null });

            typeof(CPQuery).InvokeMember("SetParameter",
                BindingFlags.InvokeMethod | BindingFlags.Instance | BindingFlags.NonPublic,
                null, query, new object[] { "p2", DBNull.Value });

            typeof(CPQuery).InvokeMember("SetParameter",
                BindingFlags.InvokeMethod | BindingFlags.Instance | BindingFlags.NonPublic,
                null, query, new object[] { "p3", new SqlParameter() });


            query = query + new StringBuilder("(ab)");
            query = query + new SqlFragment("(cd)");
            query = query + DbParameterTest.CreateParameter();


            string commandText = query.ToString();
            Console.WriteLine(commandText);

            string text = $"select 1;NULL2345@x1@x2'{guid1}''{guid2}'(ab)(cd)@id";
            Assert.AreEqual(text, commandText);

            Assert.AreEqual(6, query.Command.Parameters.Count);

            string names = (from x in query.Command.Parameters.Cast<DbParameter>()
                            let name = x.ParameterName
                            select name
                            ).ToArray().Merge(";");
            Assert.AreEqual("@x1;@x2;@p1;@p2;@p3;@id", names);
