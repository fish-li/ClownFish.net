...
namespace ClownFish.WebApi.Routing
{
	internal static class RoutingManager
	{
		...
		/// 直接可以做URL映射查找的字典表
		/// </summary>
		private static Dictionary<string, List<RoutingObject>> s_urlDict = new Dictionary<string, List<RoutingObject>>(1024, StringComparer.OrdinalIgnoreCase);

		/// <summary>
		/// 需要做正则比较的路由规则表
		/// </summary>
		private static List<RoutingObject> s_regexList = new List<RoutingObject>(1024);
