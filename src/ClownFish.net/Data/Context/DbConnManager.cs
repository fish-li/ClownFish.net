...
internal sealed class DefaultDbConnManagerImpl : IDbConnManager
{
    ...
    public DbConfig GetAppDbConfig(string connName, bool checkExist)
    {
        ...
        DbConfig dbConfig = MemoryConfig.GetDbConfig(connName)
                            ?? ConfigClient.Instance.GetAppDbConfig(connName, false)
    ...
    public DbConfig GetTntDbConfig(string tenantId, string connType, bool readonlyDB, bool checkExist)
    {
        ...
        // 优先从配置服务中获取租户库连接参数，毕竟【多租户】的连接是不可能放在本地配置文件中的
        ...
        if( dbConfig == null ) {
            ...
            dbConfig = MemoryConfig.GetDbConfig(connName) // 这里确实是个例外，主要是为了代码 “整洁”，真实场景下不太可能用上
                        ?? AppConfig.GetDbConfig(connName);
        }
