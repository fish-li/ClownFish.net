...
public static class DataLoaderFactory
{
    ...
    internal static Type GetLoaderType(Type entityType)
    {
        object value = s_table[entityType];
        if( value == null )
    ...
    internal static Type GetEntityType(Type dataloaderType)
    {
        ...
        if( dataloaderType.CanNew() == false )
            throw new ArgumentException(string.Format("类型 {0} 没有公开的无参构造函数", dataloaderType.FullName));
