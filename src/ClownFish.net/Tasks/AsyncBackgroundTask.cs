...
{
    internal static readonly int WaitSecond60 = LocalSettings.GetUInt("ClownFish_AsyncBackgroundTask_WaitSeconds1", 60);
    internal static readonly int WaitSecond66 = LocalSettings.GetUInt("ClownFish_AsyncBackgroundTask_WaitSeconds2", 66);
    ...
    private async Task WaitUntilAsync(DateTime time)
    {
        ...
        while( true ) {
            ...
            if( span1.TotalSeconds <= WaitSecond66 )
                await Wait0(span1);
            else
                // 否则，先等待 60 秒，下次循环时再计算剩余等待时间，再等待…………
                await Wait0(TimeSpan.FromSeconds(WaitSecond60));
