...
public sealed class PerformanceConfig
{
    ...
    {
        if( this.HttpExecute != int.MinValue )
            sb.Append("Performance_HttpExecute=").AppendLineRN(this.HttpExecute.ToString());

        if( this.HandleMessage != int.MinValue )
            sb.Append("Performance_HandleMessage=").AppendLineRN(this.HandleMessage.ToString());
    }
    ...
    // 毕竟 int 类型的默认值就是 0，根本没办法区分，所以为了让 LogConfiguration.MegerConfig 方法的代码好写，就取了 int.MinValue 这个默认值
